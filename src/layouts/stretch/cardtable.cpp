...
CardTable::CardTable() {
    ...
    rows->addLayout(row);
    rows->addStretch(1);                    /* Rozciągalna przestrzeń na wiersze. */
    QVBoxLayout* buttons = new QVBoxLayout();
    buttons->addStretch(1);                 /* Rozciągalna przestrzeń przed przyciskami w kolumnie. */
    buttons->addWidget(new QPushButton("Deal"));
    buttons->addWidget(new QPushButton("Shuffle"));
    buttons->addSpacing(20);                /* Stały odstęp po przyciskach. */
    QHBoxLayout* cols = new QHBoxLayout();
    setLayout(cols);
    cols->addLayout(rows);
    cols->addLayout(buttons);
    cols->addStretch(0);                    /* Jak wpłynie to na rozmiar przycisków? */
}
//end
